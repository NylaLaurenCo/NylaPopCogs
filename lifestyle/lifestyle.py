        ...
        if not await self.briefcasedisabledcheck(ctx):
            if bailbond < await userconf.briefcase():
                ...
                if await bank.can_spend(ctx.author, fee):
                    ...
                    embed = discord.Embed(
                        colour=discord.Color.from_rgb(233,60,56),
                        description=f":x: <a:hatsu_police:804202668440420353> You were caught by the police and posted bail for {amount}. You didn't have enough cash so it was taken from your bank + a {finepercent}% fine ({fee} ${humanize_number(await bank.get_currency_name(ctx.guild))}).",
                    )
                else:
