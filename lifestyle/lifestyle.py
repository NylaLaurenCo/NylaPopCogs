...
class Lifestyle(Wallet, Roulette, SettingsMixin, commands.Cog, metaclass=CompositeMetaClass):
        ...
        if not await self.walletdisabledcheck(ctx):
            if randint < await userconf.wallet():
                ...
                embed = discord.Embed(
                    ...
                    description=f"<:so_x:754605452058165289> You were caught by the police and posted bail for {amount}.",
                )
            else:
                ...
                if await bank.can_spend(ctx.author, fee):
                    ...
                    embed = discord.Embed(
                        colour=discord.Color.from_rgb(233,60,56),
                        description=f"<:so_x:754605452058165289> You were caught by the police and posted bail for {amount}. You didn't have enough cash so it was taken from your bank + a {finepercent}% fine ({fee} {await bank.get_currency_name(ctx.guild)}).",
                    )
                else:
                    await bank.set_balance(ctx.author, 0)
                    embed = discord.Embed(
                        colour=discord.Color.from_rgb(233,60,56),
                        description=f"<:so_x:754605452058165289> You were caught by the police and posted bail for {amount}. You didn't have enough cash to pay bail and are now bankrupt.",
                    )
        else:
    ...
    async def cdnotice(self, user, cooldown, job):
        response = {
            "work": f"<:so_x:754605452058165289> You won't get promoted being a kiss ass. You're on break for {cooldown}.",
            "crime": f"<:so_x:754605452058165289> Dude you're going to get arrested at that rate. Wait {cooldown} to commit another crime.",
            "slut": f"<:so_x:754605452058165289> Geez slow down. You can't slut for {cooldown}.",
            "rob": f"<:so_x:754605452058165289> The police are still on your trail. Wait {cooldown} for things to cool down.",
            "withdraw": f"<:so_x:754605452058165289> The bank is suspicious. You must wait {cooldown} to withdraw more cash.",
            "deposit": f"<:so_x:754605452058165289> Geezus, the teller is still counting your deposit from last time! Give them {cooldown} to finish up.",
        }
